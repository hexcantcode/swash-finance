...
const TTL_MS = 60_000;
let cache: { at: number; data: CohortFeed } | null = null;
...
export async function getCohortSentiment(): Promise<CohortFeed> {
  ...
  try {
    ...
    const json = (await res.json()) as {
      data?: { analytics?: { perpsMarketParticipation?: { pnlCohorts?: RawCohort[] } } };
    };
    rawCohorts = json.data?.analytics?.perpsMarketParticipation?.pnlCohorts ?? [];
  } catch {
